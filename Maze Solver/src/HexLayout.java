...
public class HexLayout implements LayoutManager, java.io.Serializable {
	...
	public Dimension preferredLayoutSize(Container parent) {
		synchronized (parent.getTreeLock()) {
			...
			int dx = insets.left + insets.right + ncols * w + (ncols - 1)
					* cgap;
			int dy = insets.top + insets.bottom + nrows * h + (nrows - 1)
					* cgap;
	...
	public void layoutContainer(Container parent) {
		synchronized (parent.getTreeLock()) {
			...
			w = (int) ((w - (ncols - 1) * cgap) / (ncols + (nrows > 1 ? 0.5f
					: 0.0f)));
	...
	public String toString() {
		return getClass().getName() + "[gap=" + cgap + ",rows=" + rows
				+ ",cols=" + cols + "]";
	}
