...
public class Main {
	...
	public static void main(String[] args) {
		...
		boolean debug = true; //can turn it off to stop printing status messages 

		StringTokenizer tokenizer;
		int ct = 0;

		InStringFile eventFileReader = new InStringFile(args[1]);
		...
		do {
			...
			if (eventType.equals("ALL_DONE"))
				if (minAllDoneTime > eventTime)
					minAllDoneTime = eventTime;
			...
			eventQueue.insertEvent(event, debug); //method we implement

		} while (!eventFileReader.endOfFile());

		System.out.println("\n" + ct + " events read and inserted in order in the eventQueue");

		eventQueue.deleteEvents(minAllDoneTime, debug); //method we implment
	}
}
